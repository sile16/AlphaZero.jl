...
##### (Plots.jl is a weak dependency - these functions are no-ops if Plots is not loaded)
#####

# Check if Plots is available at runtime
function _plots_available()
    return Base.PkgId(Base.UUID("91a5bcdd-55d7-5caf-9e0b-520d859cae80"), "Plots") in keys(Base.loaded_modules)
end

function _get_plots()
    pkgid = Base.PkgId(Base.UUID("91a5bcdd-55d7-5caf-9e0b-520d859cae80"), "Plots")
...
function plot_losses(getlosses, range, title, xlabel=:none)
    _plots_available() || return nothing
    Plots = _get_plots()
...
function learning_iter_plot(rep::Report.Learning, params::Params)
    ...
    if n == 0
        ...
        isnothing(losses_plot) && return nothing
...
function plot_iteration(
    ...
    isnothing(pplot) || push!(plots, ("iter_perfs", pplot))
    # Losses plot
...
function plot_training(
    ...
    if !isnothing(losses_fullmem)
        append!(plots,
            [entropies, nsamples, expdepth, losses_fullmem])
        append!(files,
            ["entropies", "nsamples", "exploration_depth", "loss"])
    else
        append!(plots, [entropies, nsamples, expdepth])
        append!(files, ["entropies", "nsamples", "exploration_depth"])
    end
