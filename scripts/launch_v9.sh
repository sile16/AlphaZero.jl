#!/bin/bash
# v9: joint value heads + k=7 bearoff + wildbg bootstrap + 2000 steps/iter
# Key changes from v8:
#   - k=7 bearoff table (88GB, covers 7-point home board — more positions than k=6)
#   - wildbg 1M bootstrap file (expert positions pre-fill buffer)
#   - 2000 training steps/iter (reduced from 4000 to mitigate bootstrap overfitting)
#   - Eval-excluded start positions (race_starts_tuples_no_eval.jls)
#   - Jarvis workers reduced to 8 (from 12, OOM at 12 with server+client on 64GB)
# Run on Neo: bash scripts/launch_v9.sh

set -e

JARVIS_IP="192.168.20.40"
SERVER="http://127.0.0.1:9090"
V9_DATA_DIR="/home/sile/alphazero-server-race-v9"

echo "=== Launching v9: k=7 bearoff + wildbg bootstrap ==="
echo "Key changes: k=7 bearoff, wildbg 1M bootstrap, 2000 steps/iter"
...
# Launch v9 server on Jarvis
echo "Starting v9 training server on Jarvis..."
ssh jarvis "bash -l -c 'cd /home/sile/github/AlphaZero.jl && mkdir -p $V9_DATA_DIR && nohup julia --threads 16 --project scripts/training_server.jl \
  --port 9090 \
  --data-dir $V9_DATA_DIR \
  ...
  --lr-schedule cosine \
  --lr-min 0.0001 \
  --start-positions-file /homeshare/projects/AlphaZero.jl/eval_data/race_starts_tuples_no_eval.jls \
  --eval-positions-file /homeshare/projects/AlphaZero.jl/eval_data/race_eval_2000.jls \
  --bootstrap-file /homeshare/projects/AlphaZero.jl/eval_data/bootstrap_wildbg_1M.jls \
  >> /tmp/training_server_race_v9.log 2>&1 &'"
...
# Reduced to 8 workers (from 12) to avoid OOM on 64GB Jarvis
...
ssh jarvis "bash -l -c 'cd /home/sile/github/AlphaZero.jl && nohup julia --threads 16 --project scripts/selfplay_client.jl \
  ...
  --eval-positions-file /homeshare/projects/AlphaZero.jl/eval_data/race_eval_2000.jls \
  >> /tmp/selfplay_jarvis_cpu_v9.log 2>&1 &'"
...
nohup julia --threads 30 --project scripts/selfplay_client.jl \
  ...
  --num-workers 32 \
  --client-name neo-cpu \
  >> /tmp/selfplay_neo_cpu_v9.log 2>&1 &
echo "Neo client launched (PID: $!)"

echo ""
echo "=== v9 launched! ==="
echo "Config: k=7 bearoff, wildbg bootstrap, cosine LR, PER, 2000 steps/iter, checkpoint every 5 iters"
echo "  Jarvis: 8 workers (self-play + eval-capable) via localhost"
echo "  Neo: 32 workers (self-play only) via SSH tunnel"
echo "Server data: $V9_DATA_DIR"
echo "Logs:"
echo "  Server: ssh jarvis 'tail -f /tmp/training_server_race_v9.log'"
echo "  Jarvis: ssh jarvis 'tail -f /tmp/selfplay_jarvis_cpu_v9.log'"
echo "  Neo:    tail -f /tmp/selfplay_neo_cpu_v9.log"
echo ""
