...
SERVER="http://${JARVIS_IP}:9090"
V8_DATA_DIR="/home/sile/alphazero-server-race-v8"

echo "=== Launching v8: multihead masking fix + direct IP ==="
echo "Key changes: conditional head masking, strict weight checksums, no SSH tunnel"
echo "Server: $SERVER"
...
# Verify server is up (direct IP, no tunnel)
...
# Launch Neo client (self-play only — direct IP, no tunnel)
...
echo ""
echo "=== v8 launched! ==="
echo "Config: multihead masking fix, direct IP, cosine LR, PER, 4000 steps/iter, checkpoint every 5 iters"
echo "  Jarvis: 12 workers (self-play + eval-capable) via localhost"
echo "  Neo: 32 workers (self-play only) via $SERVER"
...
echo ""
echo "Monitor: curl -s $SERVER/api/status | python3 -m json.tool"
echo "Eval:    curl -s $SERVER/api/eval/status | python3 -m json.tool"
